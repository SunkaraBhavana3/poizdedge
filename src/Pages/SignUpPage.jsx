...
const SignUpPage = () => {
  const navigate = useNavigate(); // ✅ useNavigate replaces setCurrentView
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('https://poizdedgebackend.onrender.com/api/auth/register', formData);
        ...
        <div className="lg:w-1/2 p-8 lg:p-12">
          ...
          <form className="grid grid-cols-2 gap-x-6 gap-y-5" onSubmit={handleSubmit}>
            ...
            <div className="col-span-2 flex items-center mt-2">
              <input id="agreeToTerms" name="agreeToTerms" type="checkbox" checked={formData.agreeToTerms} onChange={handleChange} required className="h-4 w-4 text-blue-600 border-gray-300 rounded" />
            ...
            <div className="col-span-2 mt-6">
              <button type="submit" style={{ backgroundColor: primaryBlue }} className="w-full flex justify-center py-3 px-4 rounded-md text-lg font-medium text-white hover:bg-blue-700 transition duration-150">
                Sign Up
              </button>
            </div>
          </form>
